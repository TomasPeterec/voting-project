...
const DashBoardVotingStatsVoters = ({ uuId }) => {
    ...
    const fetchListOfVotings = async () => {
      ...
      if (localIdToken) {
        try {
          ...
          console.log('API URL:', uuId);

          const response = await axios.get(`${apiUrl}/api/listOfVotings/${uuId}/candidates`, {
            headers: {
...
export default DashBoardVotingStatsVoters;
